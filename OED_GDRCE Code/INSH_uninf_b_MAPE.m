...
alpha_prior_params = [0.1, 0.7];
delta_prior_params = [75, 250];
beta_prior_params = [1.0, 3.0];

% End points of parameter space to consider
upper_limits=[alpha_prior_params(2), delta_prior_params(2), beta_prior_params(2)];
lower_limits=[alpha_prior_params(1), delta_prior_params(1), beta_prior_params(1)];
...
%     sprintf('No. unique group designs: %u', size(allgroupdesigns,1))
    ...
    parfor(i=1:size(current_wave_designs,1), 4)
        ...
        for k=1:size(uni_data,1)
            ...
            MAP = median(beta_params(keep));
            tmp_params = beta_params(idx1(cumulative_num(k)+1:cumulative_num(k+1)));
            
            % MAP is the max. a posteriori estimate (mode of posterior)
            %  tmp_util(k) = mean(sum(bsxfun(@rdivide, bsxfun(@minus, tmp_params, MAP), tmp_params).^2,2));
            tmp_util(k) = mean(bsxfun(@rdivide, abs(bsxfun(@minus, tmp_params, MAP)), tmp_params));
