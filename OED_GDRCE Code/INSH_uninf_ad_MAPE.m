...
upper_limits=[alpha_prior_params(2), delta_prior_params(2)];
lower_limits=[alpha_prior_params(1), delta_prior_params(1)];
...
% index on an n*n*n grid). Utility w.r.t (a,d).
...
beta_lims = [1 3];
zvals = beta_lims(1):(beta_lims(2)-beta_lims(1))/(n-1):beta_lims(2);
Zprior = 1/(beta_lims(2)-beta_lims(1)) * ones(size(zvals));
...
%     sprintf('No. unique group designs: %u', size(allgroupdesigns,1))
    ...
    % Forward simulate each of the groups according to current
    % transmission_rate. If different each time, change "transmission_rate"
    % to "transmission_rate(j)".
    for i=1:size(unique_n_d,1);
        ...
        parfor j=1:size(prior_values,1)
