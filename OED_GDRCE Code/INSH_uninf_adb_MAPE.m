...
% upper_limits=[alpha_prior_params(2), 300];
% lower_limits=[alpha_prior_params(1), 35];
...
prior_values = [alpha_params, delta_params, beta_params];

% Evaluate prior grid using indices (accumarray counts number of each
% index on an n*n*n grid)
prior_grid = accumarray([alpha_idx', delta_idx', beta_idx'], 1, [n n n]);
...
save('Output_uninf_adb_MAPE.mat')
